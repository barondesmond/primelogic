...
function timeclock_dispatch_update($tc, $dev='')
{
	...
	if ($dis = mssql_fetch_assoc($res))
	{
		...
		if ($tc['event'] == 'Traveling')
		{
			...
			$sql = "UPDATE DispTech$dev  SET DispTime = '$StartHour', TimeOn = '$StopHour' WHERE Dispatch = '" . $dis['Dispatch'] .  "' and Counter = '" . $dis['Counter'] . "' and ServiceMan = '" . $dis['ServiceMan'] . "'";
			$res = mssql_query($sql);
		...
		if ($tc['event'] == 'Working')
		{
			...
			$sql = "UPDATE DispTech$dev  SET TimeOn = '$StartHour', TimeOff = '$StopHour' WHERE Dispatch = '" . $dis['Dispatch'] . "' and Counter = '" . $dis['Counter'] . "' and ServiceMan = '" . $dis['ServiceMan'] . "'";
			$res = mssql_query($sql);
			$mes = mssql_get_last_message();
			if ($mes != '')
			{
